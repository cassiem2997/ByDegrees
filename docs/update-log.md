...
- 랜딩 히어로 목업을 두 플레이리스트 화면이 번갈아 보이는 전환 애니메이션으로 변경
  - 기존 정적 목업 버전과 회전 실험 코드는 보존하되, 실제 랜딩은 전환 목업을 사용
  - 이미지 기반 360도 회전의 어색함을 줄이고 서비스 화면 이해에 집중하도록 정리
- Vercel 사용량 초과 대응
  - 프로젝트 단위로 Next.js Image Optimization을 비활성화
  - 랜딩 히어로 목업 PNG를 WebP 정적 파일로 사전 압축해 전송량 감소
  - 세션 내 같은 경로의 페이지뷰 이벤트 중복 전송을 막아 Edge Requests와 API 함수 호출 감소
  - 방문 이벤트 전송에 `navigator.sendBeacon`을 우선 사용하고 실패 시 keepalive fetch로 fallback
- 관리자 대시보드 운영 지표 정리
  - 설명성 헤더 섹션과 기온구간 인사이트를 제거
  - 방문 -> 생성 -> 저장/공유 흐름과 실운영에 필요한 핵심 지표 중심으로 재구성
  - 생성 추이 막대그래프 집계가 실제 생성 완료 수를 표시하도록 수정
- 모바일 공유 흐름 개선
  - Web Share API로 이미지 파일 공유를 먼저 시도
  - 지원하지 않는 환경에서는 기존 X intent와 링크 복사 흐름으로 fallback
- 최적화 작업 문서화
  - Vercel 사용량, 음악 검색 캐시, 공유, 랜딩, 관리자 대시보드 최적화 내역을 `docs/optimization-summary.md`에 정리
