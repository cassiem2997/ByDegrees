# 기온별플리 / By Degrees Architecture

## 1. Product Positioning

- 대표 서비스명: `기온별플리`
- 영문 표기: `By Degrees`
- 핵심 제품 형태: 팬이 직접 곡을 고르고, 기온 구간별로 배치한 뒤, 한 장의 9:16 플레이리스트 이미지를 저장/공유하는 모바일 우선 툴

### Naming Feedback

- `By Degrees`는 더 짧고 브랜드처럼 들리며, 감성적인 서비스명으로 쓰기 좋습니다.
- 영어권에서 `degree`가 학위 의미로도 쓰이긴 하지만, 단독 브랜드명으로는 오히려 패션/에디토리얼한 느낌이 강합니다.
- 다만 이름만 보고 음악 서비스라는 점이 바로 드러나진 않을 수 있어, 대외 노출은 `기온별플리`를 메인으로 두고 `By Degrees`를 서브 브랜드처럼 붙이는 방향이 안전합니다.
- 결론: 지금 MVP에서는 `기온별플리` 메인, `By Degrees` 서브로 유지

## 2. IA

- `/`
  - `/create`로 즉시 이동
- `/create`
  - 모바일 우선 플레이리스트 이미지 생성 화면
- `/boards/[slug]`
  - 저장된 결과 이미지 미리보기
  - 이미지 저장 / X 공유
- `/admin/login`
  - 운영자 패스코드 진입
- `/admin`
  - 방문 / 저장 / 공유 / 국가 / 대륙 지표 확인

## 3. MVP User Flow

1. 사용자가 이름 또는 닉네임 입력
2. 단일 아티스트 / 복수 아티스트 옵션 선택
3. 단일 아티스트 선택 시 아티스트명 입력
4. 자동 생성된 제목 확인
5. 8개 기온 구간, 각 3칸 슬롯에 곡 선택
6. 결과 이미지를 생성하고 저장
7. X 또는 외부 SNS로 공유
8. 운영자는 `/admin`에서 지표 확인

## 4. Screen Structure

### Landing

- 현재 MVP에서는 별도 정보형 랜딩 대신 `/create` 진입 우선
- 정식 마케팅 랜딩은 추후 추가 가능

### Step Flow

- 이름 / 닉네임 입력 단계
- 아티스트 옵션 선택 단계
- 플레이리스트 편집 단계

초기 구현은 멀티 페이지보다 단일 흐름 / 단계형 UI로 묶는 것이 효율적입니다.

### Create Screen

- 결과물 규격 기준: `9:16`
- 편집 화면은 스크롤 허용
- 최종 저장 이미지는 3x8 전체가 한 장에 들어오도록 렌더
- 온도 숫자는 좌측 세로 인디케이터에만 표기
- 각 행은 3칸 고정
- 제목은 자동 생성, 사용자는 수정 불가

## 5. Title Rules

- 이름 또는 닉네임: 사용자가 입력
- 아티스트 옵션
  - 단일 아티스트
  - 복수 아티스트

### Auto Title

- 단일 아티스트: `기온별 {아티스트명} by {닉네임}`
- 복수 아티스트: `기온별 플리 by {닉네임}`

## 6. Localization

- 기본 언어: 한국어
- 헤더 우측 상단에 `KOR | ENG` 스위치 배치
- 1차 목표는 한국어 버전 완성
- 영문 번역 리소스는 구조만 열어두고, 실제 문구 반영은 후속 작업

## 7. Sharing Policy

- 이미지 저장: 최종 결과 PNG 저장
- X 공유: 링크 + 기본 해시태그 공유
- 인스타그램: prefill보다 `이미지 저장 + 앱 전환 유도` 방식 우선 검토
- 카카오: 후순위, 링크 공유 중심 검토

## 8. Tracking Policy

### Visitor

- 기준: `session_id`
- 지표
  - 일일 / 주간 / 월간 / 누적
  - 국가별 / 대륙별

### Save

- 기준: 저장 버튼 클릭
- 지표
  - 일일 / 주간 / 월간 / 누적

### Share

- 기준: 공유 액션 트리거
- 지표
  - 일일 / 주간 / 월간 / 누적

### Note

- 현재 웹 환경에서 "실제 외부 앱 업로드 완료"를 100% 보장해 잡는 것은 어렵습니다.
- 따라서 MVP에서는 실제 공유 완료가 아니라, 공유 액션 실행을 기준으로 두는 쪽이 현실적입니다.

## 9. Tech Notes

- 음악 메타데이터: Spotify Web API
- DB: Neon Postgres
- Hosting: Vercel
- Admin 보호: `ADMIN_PASSCODE` + cookie gate
- 국가 / 대륙 추적: Vercel geo headers
- Image export: client-side DOM to image
- OG image: Next.js `ImageResponse`

## 10. Current Non-Goals

- 공개 보드 탐색
- 로그인 사용자 계정 체계
- 보드 수정 기능
- 커뮤니티 기능
- 멀티 provider 동시 운영
