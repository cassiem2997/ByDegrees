...
- 영어권 국가 soft redirect와 언어 선택 쿠키
...
- 영어권 국가에서 언어 선택 쿠키가 없는 상태로 `/`에 접속하면 `/en`으로 soft redirect합니다. 사용자가 `KOR | ENG` 토글을 누르면 `bydegrees_locale` 쿠키로 선택을 기억합니다.
...
- 국가 / 대륙별 방문 지표
- 국가 / 대륙별 생성 완료 이용자 지표
- 관리자 숫자는 축약하지 않고 `5,600`처럼 전체 숫자로 표시
